...
#define CODEC_FLAG2_SPEEX_DTX   0x08000000 // discontinous transmission (DTX) (no activity -> generates 5 bits/frame, only if vbr or cng enabled)
#define CODEC_FLAG2_SPEEX_CNG   0x10000000 // confort noise generation (CNG) (no activity (codec vad) -> generates confort noise, always when vbr is enabled)
#define CODEC_FLAG2_SPEEX_ENH   0x20000000 //  perceptual enhancement (ENH) (decoder)
#define FILTER_FLAG_SPEEX_AGC     0x00000001 // Automatic Gain Control
#define FILTER_FLAG_SPEEX_VAD     0x00000002 // Voice Activity Detection (activity -> turns vad field on)
#define FILTER_FLAG_SPEEX_DENOISER     0x00000004 // Denoising
#define FILTER_FLAG_SPEEX_AGC_FIXED   0x00000008 // AGC fixed (0-> apply agc only when vad)
...
#define FF_COMPRESSION_DEFAULT -1
    ...
    /**
     ** filter list to be used in encoding/decoding
    */
    struct AVFilter* filters;
} AVCodecContext;
...
// Xiph.org Speex codec
extern AVCodec speex_decoder;
extern AVCodec speex_encoder;

// USC Intel IPP codecs
extern AVCodec g729a_decoder;
extern AVCodec g729a_encoder;
extern AVCodec g7231_decoder;
extern AVCodec g7231_encoder;
extern AVCodec g728_decoder;
extern AVCodec g728_encoder;
extern AVCodec g711alaw_decoder;
extern AVCodec g711alaw_encoder;
extern AVCodec g711ulaw_decoder;
extern AVCodec g711ulaw_encoder;
...
typedef struct AVFilterContext {
    ...
    int vad;
    int loudness;
    int echo_tail; /* number of audio samples to process in aec (echo cancellation) */
    int reserved[16]; /* for future compatibility */
    union {
        AVFrame* outf; /* out can be null, to indicate in-place transform, otherwise it will be used by nextfilter or the encoder */
        short* out;
    };
    union {
        AVFrame* extraf; /* some filters need special frames to do their processing (e.g. echo canceller needs echo signal every frame)*/
        short* extra;
    };
    void* specific;  /* some filters leave residual data to be used by the next filter */
    struct AVFilterContext* next;
} AVFilterContext;
...
typedef struct AVFilter {
    ...
    int (*process)(AVFilterContext *cur, AVFilterContext *previous, void *in);
    int (*close)(AVFilterContext *);
    int (*flush)(AVFilterContext *);
    int capabilities;
    int reserved[16];  /* for future compatibility */
...
extern AVFilter *first_avfilter;

void av_register_filter(AVFilter *filter);
AVFilter *avfilter_find_by_name(const char *name);
void avfilter_get_context_defaults(AVFilterContext *avf);
AVFilterContext *avfilter_alloc_context(void);
int avfilter_add(AVCodecContext *avctx, AVFilterContext *avf, AVFilter* filter);
int avfilter_close(AVFilterContext* avf);

extern AVFilter speex_aec_filter;
extern AVFilter speex_preproc_filter;
extern AVFilter denoiser_filter;
