...
static int avfilter_process(AVCodecContext *avctx, void *in, void**pout)
{
	int ret = 0;
	void *frame = in;

    AVFilterContext* pprev = NULL;
    AVFilterContext* p = avctx->filters;
    while (ret >= 0 && p != NULL) {
    	if (p->active) {
	    	ret = p->filter->process(p, pprev, frame);
    		if (p->out)
	    		frame = p->out;
	    }
	    pprev = p;
...
static const char* filter_context_to_name(void* ptr) {
	AVFilterContext *avf= ptr;

	if(avf && avf->filter && avf->filter->name)
		return avf->filter->name;
	else
		return "NULL";
}

static AVClass av_filter_context_class = { "AVFilterContext", filter_context_to_name, NULL };

void avfilter_get_context_defaults(AVFilterContext *s)
{
    memset(s, 0, sizeof(AVFilterContext));

    s->av_class= &av_filter_context_class;
    s->active = 1;
...
{
    AVFilterContext *avf= av_malloc(sizeof(AVFilterContext));

    if(avf)
	    avfilter_get_context_defaults(avf);
...
int avfilter_add(AVCodecContext *avctx, AVFilterContext *avf, AVFilter* filter)
{
	...
	avf->filter = filter;
	avf->avctx = avctx;
	if (filter->priv_data_size > 0)	{
		avf->priv_data = av_mallocz(filter->priv_data_size);
		if (!avf->priv_data)
			return -ENOMEM;
	}
	else
	...
	ret = avf->filter->init(avf);
	if (ret < 0) {
		av_freep(&avf->priv_data);
		return ret;
	}
                         ...
                         const short *samples)
{
    ...
    if((avctx->codec->capabilities & CODEC_CAP_DELAY) || samples){
        int ret;
        void* out = (void*) samples;
        if (samples){
            ret = avfilter_process(avctx, (void *) samples, &out);
            if (ret < 0)
                return ret;
        }
        ret = avctx->codec->encode(avctx, buf, buf_size, out);
        avctx->frame_number++;
        return ret;
    }else
        return 0;
