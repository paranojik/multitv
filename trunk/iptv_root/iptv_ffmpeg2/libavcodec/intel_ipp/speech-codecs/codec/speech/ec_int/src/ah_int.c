...
static Ipp16s bqTaps2[12]={
    1122, 2244, 1122, 16384, -1884, 6953,
...
void shiftupFft(ahState_int *state, Ipp16s *out,int len){
    LOCAL_ALIGN_ARRAY(32,Ipp32sc,spectrum,ORDER11_SPECTR_LEN,state);
    LOCAL_ALIGN_ARRAY(32,Ipp8u,pBuf,state->fftWorkBufSize,state);
    LOCAL_ALIGN_ARRAY(32,Ipp16s,sBuf,FFT_ORDER11_SRC_LEN,state);
    ippsFFTFwd_RToCCS_16s32s_Sfs((const Ipp16s *)state->histSoutS,
        (Ipp32s *)spectrum,state->FFTspec11,-8,pBuf);
    ippsMove_32sc(&spectrum[1],&spectrum[2],ORDER11_SPECTR_LEN-1);
    spectrum[1].re=1;
    spectrum[1].im=1;
    //spectrum[2].re=1;
    //spectrum[2].im=1;
    ippsFFTInv_CCSToR_32s16s_Sfs((Ipp32s *)spectrum,(Ipp16s *)sBuf,
        state->FFTspec11,8,pBuf);
    ippsCopy_16s(sBuf+FFT_ORDER11_SRC_LEN-len,out,len);
    //lowPass(state,out,out,len);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp16s,sBuf,FFT_ORDER11_SRC_LEN,state);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp8u,pBuf,state->fftWorkBufSize,state);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp32sc,spectrum,ORDER11_SPECTR_LEN,state);
}
...
static int detectHowlingFFT(ahState_int *state,Ipp16s *in,int frameSize){
    Ipp32s   cmean,ret=0;
    Ipp16s  *inp=in;
    int      i, flen=NOISE_SUBBANDS_NUM,disp[BW_DISP_LEN];
    //Ipp32s   curMagn[NOISE_SUBBANDS_NUM];
    //Ipp16s   inx[((1<<FFT_ORDER6)+2)];
    //Ipp32sc  spectrum[ORDER10_SPECTR_LEN];
    //Ipp8u    pBuf[AEC_STACK_BUF_SIZE_MAX];
    LOCAL_ALIGN_ARRAY(32,Ipp32s,curMagn,(NOISE_SUBBANDS_NUM),state);
    LOCAL_ALIGN_ARRAY(32,Ipp16s,inx,(((1<<FFT_ORDER6)+2)),state);
    LOCAL_ALIGN_ARRAY(32,Ipp32sc,spectrum,(ORDER10_SPECTR_LEN),state);
    LOCAL_ALIGN_ARRAY(32,Ipp8u,pBuf,state->fftWorkBufSize,state);

    if (frameSize>(1<<FFT_ORDER6)) {
        ret= -1;
        goto xret;
    }else{
        if ((1<<FFT_ORDER6)-frameSize>0) {
            ippsCopy_16s(in,inx,frameSize);
            ippsZero_16s(inx+frameSize,(1<<FFT_ORDER6)-frameSize);
            inp=inx;
        }
    }
    ippsFFTFwd_RToCCS_16s32s_Sfs((const Ipp16s *)inp,(Ipp32s *)spectrum,
        state->FFTspec6,-8,pBuf);
    ippsMagSquared_32sc32s_Sfs(spectrum,curMagn,flen,15);
    cmean=mean_32s(curMagn,flen);
    ippsZero_32s(disp,BW_DISP_LEN);
    if(!cmean){
        ret= 0;
        goto xret;
    }
...
xret:
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp8u,pBuf,state->fftWorkBufSize,state);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp32sc,spectrum,(ORDER10_SPECTR_LEN),state);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp16s,inx,(((1<<FFT_ORDER6)+2)),state);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp32s,curMagn,(NOISE_SUBBANDS_NUM),state);
    return ret;
}
static int detectHowlingIIR(ahState_int *state, Ipp16s *in,Ipp32s ms,int frameSize ){
    Ipp32s ams;
    //Ipp8u    pBuf[AEC_STACK_BUF_SIZE_MAX];
    int    dstLen,phase=0,dstLen2;
    LOCAL_ALIGN_ARRAY(32,Ipp16s,pBuf,(SB_FRAME_SIZE)<<2,state);
    ippsSampleUp_16s(in,frameSize,state->upDownBuf,&dstLen,4,&phase);
    own_AH_IIRBQ_16s(state->upDownBuf,pBuf,dstLen,
        bqTaps2,2,tapsFactor,state->pDly2Test);
    ippsSampleDown_16s(pBuf,dstLen,state->upDownBuf,&dstLen2,4,&phase);
    ams = ms_en(state->upDownBuf,frameSize);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp16s,pBuf,(SB_FRAME_SIZE)<<2,state);
    if ((ms*100>ams*119)){
...
static void shiftCutFreq(ahState_int *state,Ipp32sc *src, int len){
    int shift,i;
    Ipp32sc val={1,1};
    ...
    if(shift>0){
        ...
        ippsCopy_32sc(&src[state->howlLowBand],&src[state->howlLowBand+shift],
...
static void filterHowlingFFT(ahState_int *state, Ipp16s *out,int frameSize){
    //Ipp32sc  spectrum[ORDER10_SPECTR_LEN];
    //Ipp8u    pBuf[AEC_STACK_BUF_SIZE_MAX];
    LOCAL_ALIGN_ARRAY(32,Ipp32sc,spectrum,(ORDER10_SPECTR_LEN),state);
    LOCAL_ALIGN_ARRAY(32,Ipp8u,pBuf,state->fftWorkBufSize,state);
    LOCAL_ALIGN_ARRAY(32,Ipp16s,sBuf,FFT_ORDER10_SRC_LEN,state);
    ippsFFTFwd_RToCCS_16s32s_Sfs((const Ipp16s *)state->histSin,
        (Ipp32s *)spectrum,state->FFTspec10,-8,pBuf);
    shiftCutFreq(state,spectrum,ORDER10_SPECTR_LEN);
    ippsFFTInv_CCSToR_32s16s_Sfs((Ipp32s *)spectrum,sBuf,state->FFTspec10,8,pBuf);
    ippsCopy_16s(sBuf+FFT_ORDER10_SRC_LEN-frameSize,out,frameSize);
    //ippsMulC_16s_I(12.f,out,frameSize);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp16s,sBuf,FFT_ORDER10_SRC_LEN,state);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp8u,pBuf,state->fftWorkBufSize,state);
    LOCAL_ALIGN_ARRAY_FREE(32,Ipp32sc,spectrum,(ORDER10_SPECTR_LEN),state);
}
AHOpcode ah_ProcessFrame_int(ahState_int *state, Ipp16s *in, Ipp16s *out,int frameSize)
{
    int     detect;
    Ipp32s  rms;
    //detect=detectHowlingFFT(state,in);
    //return AH_DISABLE;
    if(state->mode==AH_ENABLE){
        //det = addHist(state,in);
        rms = ms_en(in,frameSize);
        if (rms<111){
            state->howlPeriod=0;
            return AH_DISABLE;
        }
        if (state->howlPeriod==0) {
            if(state->hd_mode==HD_1)
                detect=detectHowlingFFT(state,in,frameSize);
            else
                detect=detectHowlingIIR(state,in,rms, frameSize);
            if(detect<=0)
                return AH_DISABLE;
        ...
        --state->howlPeriod;
        if(state->prevHistExist)// no history?
            filterHowlingFFT(state,out,  frameSize);
        //lowPass(state,out,out,frameSize);
        return AH_ENABLE;
    }
    return AH_DISABLE;
}
int ah_GetSize_int(int frameSize, int *size,int freq,int *scratchMemSize)
{
    int specSize,specSize1,specSize2,bufSize,bufSize1,bufSize2,initSize,initSize1,
        ...
        initSize2,gsize,hsize,histSoutLen,histSoutSLen,nrSize;
    int sbStackLen,fbStackLen,sbfStackLen,ahStackLen1,ahStackLen2,ahStackLen3,
        scrLen,fdStackLen;
    *size = ALIGN(frameSize*4*sizeof(Ipp16s));/* upDownBuf */
    hsize  = ((FFT_ORDER10_SRC_LEN+frameSize-1)/frameSize)*frameSize*sizeof(Ipp16s);//histLen
    switch(frameSize) {
    case SB_FRAME_SIZE:
        histSoutLen  = SB_FRAME_SIZE*2*sizeof(Ipp16s);
        break;
    case FB_FRAME_SIZE:
    case SBF_FRAME_SIZE:
        histSoutLen  = FRAME_SIZE_10ms*2*sizeof(Ipp16s);
        break;
    default:
        return -1;
    }
    histSoutSLen  = ((FFT_ORDER11_SRC_LEN+frameSize-1)/frameSize)*frameSize*sizeof(Ipp16s);//histLen
    ippsFFTGetSize_R_16s32s(FFT_ORDER10, IPP_FFT_DIV_INV_BY_N, ippAlgHintAccurate,
        &specSize, &initSize, &bufSize);
    ippsFFTGetSize_R_16s32s(FFT_ORDER6, IPP_FFT_DIV_INV_BY_N, ippAlgHintAccurate,
    ...
    ippsFFTGetSize_R_16s32s(FFT_ORDER11, IPP_FFT_DIV_INV_BY_N, ippAlgHintAccurate,
    ...
    sbfStackLen=
        ALIGN(sizeof(Ipp32sc)*SBF_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SBF_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SBF_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SBF_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SBF_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SBF_SUBBANDS)+
        ALIGN(sizeof(Ipp32s_EC_Sfs)*SBF_SUBBANDS);

    fbStackLen=
        ALIGN(sizeof(Ipp32s)*FB_FRAME_SIZE_MAX)+
        ALIGN(sizeof(Ipp16s)*FB_FRAME_SIZE_MAX);

    sbStackLen=
        ALIGN(sizeof(Ipp32sc)*SB_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SB_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SB_SUBBANDS)+
        ALIGN(sizeof(Ipp32sc)*SB_SUBBANDS)+
        ALIGN(sizeof(Ipp32s_EC_Sfs)*SB_SUBBANDS*2)+
        ALIGN(sizeof(Ipp16s)*SB_FFT_LEN)+
        ALIGN(sizeof(Ipp16s)*SB_FFT_LEN)+
        ALIGN(sizeof(Ipp32s)*SB_SUBBANDS * 4)+
        ALIGN(sizeof(Ipp32s)*SB_SUBBANDS * 4)+
        ALIGN(sizeof(Ipp32s)*SB_FFT_LEN
        );
    ahStackLen1=
        ALIGN(sizeof(Ipp32sc)*ORDER11_SPECTR_LEN)+
        ALIGN(sizeof(Ipp8u)*gsize)+
        ALIGN(sizeof(Ipp16s)*FFT_ORDER11_SRC_LEN
        );
    ahStackLen2=
    ALIGN(sizeof(Ipp32s)*NOISE_SUBBANDS_NUM)+
    ALIGN(sizeof(Ipp16s)*(((1<<FFT_ORDER6))+2))+
    ALIGN(sizeof(Ipp32sc)*(ORDER10_SPECTR_LEN))+
        ALIGN(sizeof(Ipp8u)*gsize);
    ahStackLen3=
        ALIGN(sizeof(Ipp32sc)*ORDER10_SPECTR_LEN)+
    ALIGN(sizeof(Ipp8u)*gsize)+
    ALIGN(sizeof(Ipp16s)*FFT_ORDER10_SRC_LEN);

    fdStackLen=
        ALIGN(sizeof(Ipp32sc)*AEC_FFT_ORDER7_LEN)+
    ALIGN(sizeof(Ipp8u)*gsize)+
    ALIGN(sizeof(Ipp64s)*AEC_FFT_ORDER7_LEN);
    ...
    *scratchMemSize=scrLen;
    ...
    ippsToneDetectGetStateSize_EC_16s((IppPCMFrequency)freq, &TDsize);// freq don't used
    _ippsSubbandNoiseReduceGetSize_EC_16s(&nrSize);
    *size += ALIGN(sizeof(ahState_int))+
        ALIGN(specSize)+
        ALIGN(specSize1)+
        ALIGN(specSize2)+
        ALIGN(specSize3)+
...
int ah_Init_int(ahState_int *state,int samplingRate,int frameSize,int nlpOff)
{
    int bufSize, specSize, initSize,bufSize1, specSize1, initSize1,
        ...
        gsize, hlen,hSoutSLen,nrSize,hSoutLen;
    IppStatus status=ippStsNoErr;
    Ipp8u *pInit0,*pInit = (Ipp8u *)state,*pInit1,*pInit2,*pInit3;//;
    ippsZero_32s(state->pDly2Test,6*2);
    ippsZero_32s(state->pDly1Test,6*1);
    ippsZero_32s(state->pFDdly2,6*2);
    ippsZero_32s(state->iirDly,6);
    ippsZero_32s(state->iirDly2,6);
    ippsZero_16s(state->pDly4Test,6*4);
    initNLP(&state->nlpObj);
    state->wbNoisePwr=16;
    state->rinNoisePwr=state->wbNoisePwr;
    state->histLen = (((FFT_ORDER10_SRC_LEN+frameSize-1)/frameSize)*frameSize);
    switch(frameSize) {
    case SB_FRAME_SIZE:
        state->histSoutLen  = SB_FRAME_SIZE*2;
        break;
    case FB_FRAME_SIZE:
    case SBF_FRAME_SIZE:
        state->histSoutLen  = FRAME_SIZE_10ms*2;
        break;
    default:
        return -1;
    }
    state->histSoutSLen = (((FFT_ORDER11_SRC_LEN+frameSize-1)/frameSize)*frameSize);
    hlen = ALIGN(state->histLen*sizeof(Ipp16s));
    hSoutLen = ALIGN(state->histSoutLen*sizeof(Ipp16s));
    hSoutSLen = ALIGN(state->histSoutSLen*sizeof(Ipp16s));
    pInit += ALIGN(sizeof(ahState_int));
    state->upDownBuf = (Ipp16s *)pInit;
    pInit += ALIGN(frameSize*4*sizeof(Ipp16s));
    status=ippsFFTGetSize_R_16s32s(FFT_ORDER10, IPP_FFT_DIV_INV_BY_N, ippAlgHintAccurate,
        &specSize, &initSize, &bufSize);
    status=ippsFFTGetSize_R_16s32s(FFT_ORDER6, IPP_FFT_DIV_INV_BY_N, ippAlgHintAccurate,
        &specSize1, &initSize1, &bufSize1);
    status=ippsFFTGetSize_R_16s32s(FFT_ORDER7, IPP_FFT_DIV_INV_BY_N, ippAlgHintAccurate,
        &specSize2, &initSize2, &bufSize2);
    status=ippsFFTGetSize_R_16s32s(FFT_ORDER11, IPP_FFT_DIV_INV_BY_N, ippAlgHintAccurate,
        &specSize3, &initSize3, &bufSize3);
    pInit0 = pInit;
    ...
    status=ippsFFTInit_R_16s32s(&(state->FFTspec10), FFT_ORDER10, IPP_FFT_DIV_INV_BY_N,
        ippAlgHintAccurate, pInit0, pInit);
    status=ippsFFTInit_R_16s32s(&(state->FFTspec6), FFT_ORDER6, IPP_FFT_DIV_INV_BY_N,
    ...
    status=ippsFFTInit_R_16s32s(&(state->FFTspec11), FFT_ORDER11, IPP_FFT_DIV_INV_BY_N,
    ...
    ippsToneDetectGetStateSize_EC_16s((IppPCMFrequency)samplingRate, &gsize);

    state->td.tdr = (IppsToneDetectState_EC_16s *)pInit;
    pInit += ALIGN(gsize);
    state->td.tds = (IppsToneDetectState_EC_16s *)pInit;
    pInit += ALIGN(gsize);
    ippsToneDetectInit_EC_16s(state->td.tdr, (IppPCMFrequency)samplingRate);
    ippsToneDetectInit_EC_16s(state->td.tds, (IppPCMFrequency)samplingRate);
    state->td.td_thres = (Ipp64s)(IPP_AEC_dBtbl[35-10]*IPP_AEC_dBtbl[35-10]);
    state->td.td_resr = state->td.td_ress = 0;
    state->td.td_sil_time=0;
    state->td.td_sil_thresh=250;
    _ippsSubbandNoiseReduceGetSize_EC_16s(&nrSize);
    state->nr = (IppsSubbandNoise_Reduce_AEC_16s *)pInit;
    pInit += ALIGN(nrSize);
    state->nrSin = (IppsSubbandNoise_Reduce_AEC_16s *)pInit;
    pInit += ALIGN(nrSize);
    state->histRin=(Ipp16s *)pInit;
    pInit += hlen;
    state->histSin=(Ipp16s *)pInit;
    pInit += hlen;
    state->histSout=(Ipp16s *)pInit;
    pInit += hSoutLen;
    state->histSoutS=(Ipp16s *)pInit;
    pInit += hSoutSLen;
    state->samplingRate = (Ipp16s)samplingRate;
    frameSize = frameSize;
    state->shift = 0;
    state->mode = AH_DISABLE;
    state->histSinCurLen = nlpOff;
    state->histRinCurLen = nlpOff;
    ...
    state->sl_pwr=0;
    state->nsi_pwr=0;
    state->sqrtNoisePwr=aec_i_sqrt((Ipp32u)state->wbNoisePwr);
    ippsZero_16s(state->histRin,state->histLen);
    ippsZero_16s(state->histSin,state->histLen);
    ippsZero_16s(state->histSout,state->histSoutLen);
    ippsZero_16s(state->histSoutS,state->histSoutSLen);
    ippsZero_64s(state->noiseCurMagn,AEC_FFT_ORDER7_LEN);

    state->noiseMax=0;
    _ippsSubbandNoiseReduceInit_EC_16s(state->nr);
    _ippsSubbandNoiseReduceInit_EC_16s(state->nrSin);
...
void formFFTin(ahState_int *state, Ipp16s *sout, int len){
    /* renew the history */
    ippsMove_16s(state->histSoutS+len,state->histSoutS,
        state->histSoutSLen-len);
    ippsCopy_16s(sout,state->histSoutS+state->histSoutSLen-len,len);
   return;
}
